...
//Подключаем модели
const { userModel, Avito_ObjectModel } = require('../models/models');

class AvitoObjectController{
	...
	async update(req, res){
		...
		if(!valid){
			...
			if(user!=null){
				...
				let request=(await user.getAvito_Requests({
				...
				if(request!=null){
					...
					let object=(await request.getAvito_Objects({
	...
	async getAll(req, res){
		...
		const totalObjects=await request.countAvito_Objects();
		
		//Получаем список объектов в соответствии с пагинацией
		let objectsInThisPage=await request.getAvito_Objects({
		...
		let listOfMyAvitoObjects=objectsInThisPage.map((AvitoObject)=>{
					
			let formattedAvitoObject={
							
					id:AvitoObject.id,
					
					linkOnSite:AvitoObject.linkOnSite,

					name:AvitoObject.name,
					
					description:AvitoObject.description,
					
					viewed:AvitoObject.viewed,
					
					price:AvitoObject.price,
					
					detectionTime:AvitoObject.detectionTime,
					
					creationTime:AvitoObject.creationTime,
									
			};

			return formattedAvitoObject;
...
module.exports = new AvitoObjectController();
